...
block {
    ...
    if (Tezos.sender = removed_swap.owner) then skip else failwith("Only the owner can cancel the swap");
    const new_store : swap_storage = record [
...
block {
    ...
    if (Tezos.sender = accepted_swap.owner) then failwith("The owner should not be able to accept swap") else skip;
    const new_store : swap_storage = record [
        swaps = Big_map.remove (token_id, store.swaps);
        token_address = store.token_address;
        metadata = store.metadata;
